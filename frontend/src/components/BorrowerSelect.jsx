  ...
  const toggleBorrower = (member) => {
    setSelectedBorrowers((prevSelected) => {
      const isSelected = prevSelected.some(
        (borrower) => borrower.borrowerId === member.id
      );
      let newSelection;
      ...
      if (expenseType === 'equal') {
        ...
        if (isSelected) {
          ...
          newSelection = [
            ...prevSelected,
            { borrowerId: member.id, amount: 0, percentage: 0 },
          ];
        }
      }
  ...
  return (
    <div className="mb-6">
      ...
      <div className="dropdown w-full max-w-sm">
        <label tabIndex={0} className="btn bg-base-200 btn-outline btn-block">
          Select Borrowers
        </label>
        <ul
          tabIndex={0}
